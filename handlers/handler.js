...
const { loadPrefixes, guildPrefixes } = require("../functions/loadPrefixes"); // Importing the load prefixes function
const addMessages = require("../functions/handleMessages"); // Import handle messages
const checkTags = require("../functions/checkTags");
...
module.exports = async (client, message) => {
  try {
    ...
    // Load prefixes after all basic checks are done
    loadPrefixes(client);
    ...
    //Or find it in the custom prefixes data
    let prefix = guildPrefixes[message.guild.id] || config.prefix;
    ...
    checkTags(client, message, args, cmd);
    ...
    if (command) {
        ...
        if (
          ...
          return message.channel.send(
            new Discord.MessageEmbed()
              ...
              .setDescription(
                `You need these Permissions: \`${command.permissions.join(
                  "`, ``"
                )}\``
              )
          );
        ...
        if (!message.guild.me.hasPermission(required_perms)) {
          ...
          return message.channel.send(
            new Discord.MessageEmbed()
              ...
              .setDescription(
                "Please give me just `ADMINISTRATOR`, because I need it to delete Messages, Create Channel and execute all Admin Commands.\n If you don't want to give me them, then those are the exact Permissions which I need: \n> `" +
                  required_perms.join("`, `") +
                  "`"
              )
          );
      ...
      if (config.return_if_unknown_command === true)
        return message.channel
          .send(
            new Discord.MessageEmbed()
              .setColor(ee.wrongcolor)
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(`❌ Unknown command, try: **\`${prefix}help\`**`)
              .setDescription(`Make sure you enter a valid command`)
          )
